 ...
 * @author GarandaTech
 */
@WebServlet(urlPatterns = {"/AddSoldProvider"})
public class AddSoldProvider extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                if (!amount_DJEZZY.isEmpty()) {
                    ...
                    System.out.println("AddSold.processRequest() provider =" + provider);

                    db.updateVirtualBalanceProvider(userID, provider, idtrader, Integer.parseInt(id_DJEZZY), Double.parseDouble(amount_DJEZZY));
                }
                if (!amount_MOBILIS.isEmpty()) {
                    System.out.println("AddSold.processRequest() Mobilis" + amount_MOBILIS);
                    db.updateVirtualBalanceProvider(userID, provider, idtrader, Integer.parseInt(id_MOBILIS), Double.parseDouble(amount_MOBILIS));
                }
                if (!amount_OOREDOO.isEmpty()) {
                    System.out.println("AddSold.processRequest() Ooredoo" + amount_OOREDOO);

                    db.updateVirtualBalanceProvider(userID, provider, idtrader, Integer.parseInt(id_OOREDOO), Double.parseDouble(amount_OOREDOO));
                }
                response.sendRedirect("view/listClientGro.jsp?id=15&succesSold");
            } catch (Exception e) {
                response.sendRedirect("view/listClientGro.jsp?id=15&erreur");
            }
        }
    }
