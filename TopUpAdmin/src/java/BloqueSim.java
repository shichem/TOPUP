 ...
 * @author GarandaTech
 */
@WebServlet(urlPatterns = {"/BloqueSim"})
public class BloqueSim extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            StatusInfo si = util.getStatusInfo_by_statusInfoDesc(session, staticVars.status_ENT_Bloque, "");
            offerInfo.setStatusInfo(si);
            info_Util.updateSimInfo(offerInfo, session);
            session.getTransaction().commit();
            session.close();
            response.sendRedirect("view/listSim.jsp?bloque");
        } catch (Exception e) {
